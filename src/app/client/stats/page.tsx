...
export default async function ClientStatsPage({
    ...
    const statsByProduct = orderItems.reduce((acc, item) => {
        ...
        acc[product.id].totalSpent += Number(item.price) * item.quantity;
        return acc;
    }, {} as Record<string, { name: string, units: number, totalSpent: number, category: string }>);
    ...
    const totalSpent = Object.values(statsByProduct).reduce((sum, p) => sum + p.totalSpent, 0);
